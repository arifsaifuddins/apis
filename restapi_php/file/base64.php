...
if ($method == 'POST') {
  ...
  if (isset($_POST['nama']) && isset($_POST['jurusan']) && isset($_POST['usia']) && isset($_POST['alamat'])) {
    ...
    // tangkap file base64
    $foto_str = $_POST['fotostr']; // string
    // ext foto
    $ext_foto = $_POST['extfoto'];

    // ubah string jadi file
    $foto = base64_decode($foto_str);
    // simpan hasil decode base64
    file_put_contents('../img/' . $nama . '.' . $ext_foto, $foto);
    // buat nama foto
    $nama_foto = $nama . '.' . $ext_foto;
